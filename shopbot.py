...
if __name__ == "__main__":
    ...
    async def run_bot_with_retry():
        """เริ่มบอทพร้อมการจัดการ Rate Limiting และ Connection Management"""
        max_retries = 3
        retry_delay = 30  # ลดเวลารอลง
        
        for attempt in range(max_retries):
            try:
                print(f"🤖 กำลังเริ่มบอท (ครั้งที่ {attempt + 1}/{max_retries})...")
                
                # ตรวจสอบ Token ก่อน
                if not TOKEN or TOKEN == "YOUR_BOT_TOKEN":
                    print("❌ Discord Token ไม่ถูกต้อง! กรุณาตั้งค่า DISCORD_TOKEN ใน Environment Variables")
                    return
                
                # เริ่มบอทพร้อมจัดการ session อย่างถูกต้อง
                async with bot:
                    await bot.start(TOKEN)
                break  # ถ้าสำเร็จให้หยุดลูป
                
            except discord.HTTPException as e:
                if e.status == 429:  # Rate Limited
                    print(f"⚠️ Rate Limited - รอ {retry_delay} วินาที แล้วลองใหม่...")
                    if attempt < max_retries - 1:
                        await asyncio.sleep(retry_delay)
                        retry_delay = min(retry_delay * 1.5, 300)  # จำกัดเวลารอสูงสุด 5 นาที
                        continue
                    else:
                        print("❌ เกินจำนวนครั้งที่กำหนด - Discord API ยังคง Rate Limited")
                        print("💡 ลองใหม่ในภายหลัง หรือตรวจสอบว่าบอทถูก Rate Limited จากการใช้งานที่อื่น")
                        raise
                elif e.status == 401:
                ...
                if "Cannot connect to host discord.com" in str(e):
                    print("⚠️ ปัญหาการเชื่อมต่อเครือข่าย - กำลังลองใหม่...")
                    if attempt < max_retries - 1:
                        await asyncio.sleep(retry_delay)
                        retry_delay = min(retry_delay * 1.5, 120)
                        continue
                    else:
                        print("❌ ไม่สามารถเชื่อมต่อ Discord ได้ - ตรวจสอบการเชื่อมต่ออินเทอร์เน็ต")
                        raise
                else:
                    raise
    ...
    try:
        ...
        # เพิ่ม random delay เพื่อหลีกเลี่ยง Rate Limiting ร่วมกับบอทอื่น
        import random
        delay = random.uniform(5, 15)  # รอ 5-15 วินาทีแบบสุ่ม
        print(f"⏳ รอ {delay:.1f} วินาที เพื่อหลีกเลี่ยง Rate Limiting...")
        time.sleep(delay)
        
        # เริ่มการทำงานของบอทแบบง่าย
